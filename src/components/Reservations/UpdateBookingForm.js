...
function UpdateBookingForm({ id }) {
  ...
  React.useEffect(() => {
    axios
      .get(`https://61f92889783c1d0017c449b5.mockapi.io/api/v1/bookings/${id}`)
      .then((res) => setBookingData(res?.data));
  });

  const onSubmit = (data) => {
    data["bookingDate"] = currentDate?.toLocaleDateString(undefined, {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    data["startDate"] = value[0]?.toLocaleDateString(undefined, {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    data["endDate"] = value[1]?.toLocaleDateString(undefined, {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    axios
      .put(
        `https://61f92889783c1d0017c449b5.mockapi.io/api/v1/bookings/${id}`,
        data
      )
      .then((res) => {
        if (res?.data) {
        ...
        <Grid item xs={4} sm={8} md={12} lg={12}>
          <FormControl variant="filled" sx={{ width: "100%" }}>
            ...
            <input
              className="input"
              placeholder="Amount"
              defaultValue={bookingData?.amount}
              {...register("amount")}
            />
          </FormControl>
        </Grid>

...
function UpdateRoomForm({ id }) {
  ...
  React.useEffect(() => {
    axios
      .get(`https://61f92889783c1d0017c449b5.mockapi.io/api/v1/rooms/${id}`)
  ...
  const onSubmit = (data) => {
    axios
      .put(
        `https://61f92889783c1d0017c449b5.mockapi.io/api/v1/rooms/${id}`,
        data
      )
      .then((res) => {
        if (res?.data) {
        ...
        <Grid item xs={4} sm={8} md={12} lg={12}>
          <FormControl variant="filled" sx={{ width: "100%" }}>
            ...
            <input
              ...
              defaultValue={roomData?.hotelName}
              {...register("hotelName")}
            />
          </FormControl>
        </Grid>
      ...
      >
        ...
        <Grid item xs={4} sm={8} md={6} lg={6}>
          <FormControl variant="filled" sx={{ width: "100%" }}>
            ...
            <input
              className="input"
              placeholder="Room Telephone"
              defaultValue={roomData?.telephone}
              {...register("telephone")}
            />
          </FormControl>
        </Grid>
